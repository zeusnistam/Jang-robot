...
TOKEN = '8654179425:AAEShXnD69Um52wEApHDfzoM6h0G_dkkhdk' # توکن رباتت بزار اینجا
...
async def admin_decl_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data.startswith("adm_decl_ok_"):
        ...
        pub_msg = (
            ...
            f"🗣️ {decl['text']}\n\n"
            f"{'━'*22}\n"
            f"🏢 این بیانیه توسط سازمان جهانی بیانیه به صورت رسمی تایید شده است ✅"
        )
...
def admin_menu_keyboard():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("📋 لیست کاربران", callback_data="adm_users_list_0")],
...
async def admin_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if awaiting == "ban_user_id":
        ...
        if not target:
            await update.message.reply_text("❌ این کاربر وجود نداره.")
            raise ApplicationHandlerStop()
        ...
        await update.message.reply_text(
            ...
            f"کشور {target.get('country', 'نامشخص')} اکنون بدون صاحب است.",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("👑 پنل ادمین", callback_data="admin_panel")]])
        )
        ...
        try:
            await context.bot.send_message(
                ...
                "اگه فکر می‌کنی این تصمیم اشتباه است، با ادمین تماس بگیر.",
                parse_mode="Markdown"
            )
        except Exception:
            pass
...
async def admin_setcountry(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    args = context.args
    if len(args) < 2:
        await update.message.reply_text("❌ فرمت درست:\n`/setcountry آیدی_عددی نام_کشور`\n\nمثال: `/setcountry 1601379026 ایران`", parse_mode="Markdown")
        return
    try:
    ...
    holder = get_player_by_country(country_name)
    if holder and holder.get("user_id") != target_id:
        await update.message.reply_text(f"❌ کشور «{country_name}» قبلاً توسط یه کاربر دیگه گرفته شده.")
        return
    ...
    await update.message.reply_text(
        f"✅ کشور «{country_name}» به کاربر `{target_id}` اختصاص داده شد.",
        parse_mode="Markdown"
    )
    try:
        await context.bot.send_message(
            target_id,
            f"🎖️ فرمانده، کشور *{country_name}* به تو اختصاص داده شد!\n\nبرای ورود به بازی /start بزن.",
            parse_mode="Markdown"
        )
    except Exception:
        pass
...
async def admin_setgroup(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    args = context.args
    if len(args) < 2:
        await update.message.reply_text("❌ فرمت درست:\n`/setgroup آیدی_عددی نام_گروهک`\n\nمثال: `/setgroup 1601379026 داعش`", parse_mode="Markdown")
        return
    try:
        target_id = int(args[0])
    except ValueError:
    ...
    holder = get_player_by_country(group_name)
    if holder and holder.get("user_id") != target_id:
        await update.message.reply_text(f"❌ گروهک «{group_name}» قبلاً توسط یه کاربر دیگه گرفته شده.")
        return
    ...
    await update.message.reply_text(
        f"✅ گروهک «{group_name}» به کاربر `{target_id}` اختصاص داده شد.",
        parse_mode="Markdown"
    )
    try:
        await context.bot.send_message(
            target_id,
            f"🎖️ فرمانده، گروهک *{group_name}* به تو اختصاص داده شد!\n\nبرای ورود به بازی /start بزن.",
            parse_mode="Markdown"
        )
    except Exception:
        pass
...
def get_banned_users() -> list:
    conn = sqlite3.connect("game.db")
    c = conn.cursor()
    c.execute("SELECT value FROM settings WHERE key='banned_users'")
    row = c.fetchone()
    conn.close()
...
def set_banned_users(banned_list: list):
    conn = sqlite3.connect("game.db")
    c = conn.cursor()
    c.execute("INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)", ("banned_users", json.dumps(banned_list)))
    conn.commit()
    conn.close()
...
def ban_user(user_id: int):
    ...
    if user_id not in banned:
        ...
        set_banned_users(banned)
    p = get_player_by_id_full(user_id)
    if p and p.get("country"):
        update_player(user_id, {"country": None})
...
async def admin_users_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        page = int(query.data.replace("adm_users_list_", ""))
    ...
    conn.row_factory = sqlite3.Row
    c = conn.cursor()
    c.execute("SELECT user_id, username, country FROM players WHERE country IS NOT NULL ORDER BY user_id DESC")
    ...
    if total_users == 0:
        await query.edit_message_text(
            "📋 *لیست کاربران*\n━━━━━━━━━━━━━━━━\n\n❌ هنوز کاربری با کشور ثبت‌شده نیست.",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 برگشت", callback_data="admin_panel")]]),
            parse_mode="Markdown"
        )
        return
    
    total_pages = (total_users + 6) // 7
    ...
    for pu in page_users:
        text += f"👤 {pu['username'] or 'بی‌نام'} | `{pu['user_id']}`\n🌍 {pu['country']}\n\n"
    
    text += f"صفحه {page + 1}/{max(1, total_pages)}"
    
    kbd = []
    if page > 0:
        kbd.append(InlineKeyboardButton("⬅️ قبلی", callback_data=f"adm_users_list_{page-1}"))
    if page < total_pages - 1:
        kbd.append(InlineKeyboardButton("➡️ بعدی", callback_data=f"adm_users_list_{page+1}"))
    ...
    kbd_full.append([InlineKeyboardButton("🚫 بن کاربر", callback_data="adm_ban_prompt")])
    kbd_full.append([InlineKeyboardButton("🔙 برگشت", callback_data="admin_panel")])
    
    await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(kbd_full), parse_mode="Markdown")
...
async def admin_ban_prompt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id not in ADMIN_IDS:
        ...
        return
    await query.answer()
    context.user_data["admin_await"] = "ban_user_id"
    await query.edit_message_text(
        "🚫 *بن کاربر*\n━━━━━━━━━━━━━━━━\n"
        "✍️ آیدی عددی کاربر رو وارد کن:",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 لغو", callback_data="adm_users_list_0")]]),
        parse_mode="Markdown"
    )
...
async def admin_ban(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    args = context.args
    if not args:
        await update.message.reply_text("❌ فرمت درست:\n`/ban آیدی_عددی`", parse_mode="Markdown")
        return
    try:
        target_id = int(args[0])
    except ValueError:
    ...
    ban_user(target_id)
    await update.message.reply_text(f"✅ کاربر `{target_id}` بن شد.", parse_mode="Markdown")
    try:
        await context.bot.send_message(target_id, "🚫 شما از ربات بن شده‌اید!")
    except Exception:
        pass
...
async def admin_unban(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    args = context.args
    if not args:
        await update.message.reply_text("❌ فرمت درست:\n`/unban آیدی_عددی`", parse_mode="Markdown")
        return
    try:
        target_id = int(args[0])
    except ValueError:
    ...
    await update.message.reply_text(f"✅ بن کاربر `{target_id}` برداشته شد.", parse_mode="Markdown")
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(admin_users_list, pattern=r"^adm_users_list_\d+$"), group=0)
    app.add_handler(CallbackQueryHandler(admin_ban_prompt, pattern="^adm_ban_prompt$"), group=0)
    app.add_handler(MessageHandler(
