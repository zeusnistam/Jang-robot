...
ATTACK_LOG_CHANNEL = -1003799634960  # چنل رین وار

async def send_normal_attack_log(context, attacker_name, defender_country, damage_percent):
    """ارسال پیام حمله عادی به چنل لاگ"""
    message = f"""🔴 **عملیات حمله عادی کلید خورد** 🔴
...
💣 **میزان خسارت:**
  📊 {damage_percent}٪ از منابع هدف

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
📌 **وضعیت:** ✅ حمله موفقیت‌آمیز
⏰ **زمان:** در حال حاضر

🌀 بروزرسانی‌های فوری به دنبال می‌رود..."""
    
    try:
        await context.bot.send_message(
            chat_id=ATTACK_LOG_CHANNEL,
            text=message,
            parse_mode="Markdown"
        )
...
async def attack_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ارسال پیام حمله عادی به چنل لاگ
    attacker_player = get_player_by_country(result["attacker_country"])
    defender_player = get_player_by_country(result["defender_country"])
    if attacker_player and defender_player:
        try:
            await send_attack_log_to_channel(
                context,
                attacker_player.get("username", f"کاربر {attacker_player['user_id']}"),
                defender_player.get("country", "کشور نامشخص"),
                attack_type="normal",
                damage=round(result.get("damage_percent", 0))
            )
        except Exception as e:
            logger.error(f"Send attack log error: {e}")
...
async def alliance_attack_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ارسال پیام حمله گروهی به چنل لاگ
    try:
        alliance = get_alliance(user_id)
        attacker_player = get_player_by_country(result["attacker_country"])
        if alliance and attacker_player:
            await send_attack_log_to_channel(
                context,
                f"{alliance.get('name', 'اتحادی نامشخص')} (لیدر: {attacker_player.get('username', 'نامشخص')})",
                result.get("defender_country", "کشور نامشخص"),
                attack_type="alliance",
                damage=round(result.get("damage_percent", 0))
            )
    except Exception as e:
        logger.error(f"Send group attack log error: {e}")
